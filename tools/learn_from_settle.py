#!/usr/bin/env python3
"""After a settle, retune the book that fired. Quiet does not call this.

Every filled ticket — sports, crypto15m, and crypto_scalp — feeds the next score.
Scalp also nudges min_dip / take / stop from P&L.
...
LEARN_TAPE = ROOT / "ledger" / "learn_tape.jsonl"
...
def main() -> None:
    ...
    if book_kind not in {"sports", "crypto15m", "crypto_scalp"}:
        raise SystemExit("score.book_kind must be sports, crypto15m, or crypto_scalp")
    ...
    gate = gate_pct_for(book_kind=book_kind)
    scalp_after = None
    if book_kind == "crypto_scalp":
        scalp_after = apply_scalp_learn(prior.get("scalp") or DEFAULT_SCALP, settle["result"])
        prior["scalp"] = scalp_after
    notes = (
        f"{settle['result']} {book_kind} ticket {settle.get('ticket_id')}. "
        f"{'+' if settle['result'] == 'WON' else '-'}0.02 on {', '.join(deltas) or 'none'}; "
        f"espn frozen on crypto books, crypto frozen on sports. "
        f"Gate stays {gate:g}% on this book."
    )
    event = {
        "ts": settle.get("ts"),
    ...
    if scalp_after:
        event["scalp"] = scalp_after
    if score.get("features"):
        event["features"] = score["features"]
    if args.dry_run:
    ...
    tape_path = Path(os.environ.get("LEARN_TAPE_PATH", LEARN_TAPE))
    tape_path.parent.mkdir(parents=True, exist_ok=True)
    with tape_path.open("a") as fh:
        fh.write(json.dumps(tape_row) + "\n")
    append(event)
    print(json.dumps({"book_kind": book_kind, "deltas": deltas, "weights": after, "scalp": scalp_after}))
