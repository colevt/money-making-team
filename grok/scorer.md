...
- Onchain scalp (`crypto_scalp`): from `scalp.py` only. `edge_pct>=0.35` AND `ask<1.00`. BUY the dip; SELL the same lot — do not add a second long. One position per token. Small `TICKET_USD` (default $1), many round-trips.
